...
// Used by the public token profile page when a token isn't in SolVerify DB
// =============================================================================
...
export async function GET(req: NextRequest, { params }: { params: { address: string } }) {
  try {
    ...
    if (!pools || pools.length === 0) {
      return jsonError("No live data found for this token", 404, "NO_DATA");
    }
    ...
    // Risk scan
    const risk = await scanRisk(params.address);
