...
export async function POST(req: NextRequest, { params }: { params: { address: string } }) {
  try {
    ...
    if (file.size > MAX_BANNER_BYTES) return jsonError("File too large (10MB max)", 400, "TOO_LARGE");

    const ext = file.name.split(".").pop() || "png";
    const path = `${token.id}/banner-${Date.now()}.${ext}`;
    const buf = new Uint8Array(await file.arrayBuffer());
    const { error: upErr } = await db.storage
      .from("token-assets")
      .upload(path, buf, { contentType: file.type, upsert: true });
    if (upErr) return jsonError("Upload failed", 500, "STORAGE_ERROR");
    const { data: pub } = db.storage.from("token-assets").getPublicUrl(path);
    await db.from("tokens").update({ banner_url: pub.publicUrl }).eq("id", token.id);
